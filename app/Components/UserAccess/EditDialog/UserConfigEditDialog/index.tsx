...
import UserConfigFormFragment from "./form-fragment";
...
export default function UserConfigEditDialog({
  data,
  asIcon = false,
  employees = [],
  company_id,
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      const userConfig: IUserConfig = {
        ...
        password: fd.get("end_date") as string,
      };

      setLoading(true);

      try {
        const apiRes = await fetch(`/api/user-access/user-config`, {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className={DialogContentWidth}>
        <DialogHeader>
          <DialogTitle>
            {data ? "Update" : "Add"} User Configuration
          </DialogTitle>
          <DialogDescription>
