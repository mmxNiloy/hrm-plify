...
import updateSystemUser from "@/app/(server)/actions/user/system-user/update-system-user.controller";
...
} from "@/schema/form/system-user.schema";
...
const FormSchema = UpdateSystemUserSchema;

type FormType = z.infer<typeof FormSchema>;

export default function SystemUserEditForm({
  permissions,
  onSuccess,
  data,
}: Props) {
  ...
  const form = useForm<FormType>({
    ...
    defaultValues: {
      ...
      email: data.email,
      read_permissions: data.access_permissions
        .filter((item) => item.permission.permission_name.endsWith("read"))
        .map((item) => item.permission_id.toString()),
      write_permissions: data.access_permissions
        .filter((item) => item.permission.permission_name.endsWith("create"))
        .map((item) => item.permission_id.toString()),
      update_permissions: data.access_permissions
  ...
  const onSubmit = useCallback(
    (values: FormType) => {
      startSubmit(async () => {
        const result = await updateSystemUser(data.user_id, values);
        if (result.error) {
          toast.error("Failed to upate user!");
        } else {
