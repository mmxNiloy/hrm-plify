...
import createEmployeeDocument from "@/app/(server)/actions/company/document/create-employee-document.controller";
import updateCompanyDocument from "@/app/(server)/actions/company/document/update-company-document.controller";
import updateEmployeeDocument from "@/app/(server)/actions/company/document/update-employee-document.controller";
...
interface Props extends IFormFragmentProps<IEmployeeDoc> {
  onSuccess?: () => void;
  employeeId: string;
}
...
export default function EmployeeDocumentForm({
  ...
  const onSubmit = useCallback(
    (values: FormType) => {
      startSubmit(async () => {
        try {
          ...
          let operation;
          if (data) {
            operation = updateEmployeeDocument(data.doc_id.toString(), {
              ...values,
              doc_link: docLink,
            });
          } else {
            operation = createEmployeeDocument(employeeId, {
              ...values,
              doc_link: docLink,
            });
          }
    ...
    [employeeId, data, onSuccess, router],
  );
