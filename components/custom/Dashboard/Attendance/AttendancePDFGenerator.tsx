...
export default function AttendancePDFGenerator({
  ...
  const generatePdf = useCallback(async () => {
    ...
    const groupedData = data.reduce<Record<string, IAttendanceRecord[]>>(
      (acc, record) => {
    ...
    for (const [employeeId, records] of Object.entries(groupedData)) {
      ...
      const numberOfAbsences = records.filter((r) => r.is_present == 0).length;
      const numberOfHolidays = records.filter((r) => r.is_present == 3).length;
    ...
    setLoading(false);
  }, [company, toast, loadImage, data, employee]);

  return (
    <Button
      disabled={loading || (data?.length ?? 0) < 1}
      onClick={() => {
        if (employee) {
          generatePdf();
        }
