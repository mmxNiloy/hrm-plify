...
export default function VisaNotificationReportGenerator({
  data,
  company,
}: {
  ...
  const generateEmployeePDF = useCallback(() => {
    const diff = data.visa_brp?.expiry_date
      ? dateDiffInDays(new Date(data.visa_brp.expiry_date), new Date())
      : -1;
    ...
    if (diff > 0) {
      ...
      doc.text(`Visa Expired!`, margin + 5, y);
    }
    y += 15;
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `Subject: Right to Work Documentation - Temporary Visa ${
    ...
    doc.setFontSize(10);
    const content = [
      `Further to your employment on a temporary visa, I am writing to remind you that this visa is ${
        diff > 0 && data.visa_brp?.expiry_date
          ? `due to expire on ${new Date(
              data.visa_brp.expiry_date
    ...
    const paragraphs = [
      ...
      "of any application at the time of your visa expiring or at 5 monthly intervals dependent on the checking requirements",
      ...
      "Otherwise, we will have no option but to review your ongoing right to work when your current visa expires. A failure to",
    ...
    doc.save(
      `${getFullNameOfUser(data.user).replace(
        " ",
        "_"
      )}_Visa_Notification_${Date.now()}.pdf`
    );
