...
export default function PassportNotificationReportGenerator({
  data,
  company,
}: {
  ...
  const generateEmployeePDF = useCallback(() => {
    const diff = data.emp_passport?.expiry_date
      ? dateDiffInDays(new Date(data.emp_passport.expiry_date), new Date())
      : -1;
    ...
    if (diff > 0) {
      ...
      doc.text(`Passport Expired!`, margin + 5, y);
    }
    y += 15;
    ...
    doc.setFont("helvetica", "bold");
    doc.text(
      `Subject: Right to Work Documentation - Temporary Passport ${
    ...
    doc.setFontSize(10);
    const content = [
      `Further to your employment on a temporary Passport, I am writing to remind you that this Passport is ${
        diff > 0 && data.emp_passport?.expiry_date
          ? `due to expire on ${new Date(
              data.emp_passport.expiry_date
    ...
    const paragraphs = [
      ...
      "of any application at the time of your Passport expiring or at 5 monthly intervals dependent on the checking requirements",
      ...
      "Otherwise, we will have no option but to review your ongoing right to work when your current Passport expires. A failure to",
    ...
    doc.save(
      `${getFullNameOfUser(data.user).replace(
        " ",
        "_"
      )}_Passport_Notification_${Date.now()}.pdf`
    );
