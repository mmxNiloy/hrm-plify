...
export default function StatusFilter() {
  const [departmentStatus, setDepartmentStatus] = useQueryState(
    "status",
    searchParamsParsers.status
  );

  const statusList: (typeof departmentStatus)[] = useMemo(
    () => ["all", "1", "0"],
    []
  );
    ...
    <div className="flex border rounded-md border-blue-600 overflow-clip">
      {statusList.map((status) => (
        <Button
          ...
          onClick={() => setDepartmentStatus(status)}
        >
          {departmentStatus === status && <Check className={cn("size-3")} />}
